...
class PreferenceManager {
  private cache = createStorage<PreferencesStorage>('local', '');
   ...
   */
  clearCache = () => {
    Object.values(STORAGE_KEYS).forEach((key) => this.cache.remove(key));
  };
  ...
  initPreferences = async ({ namespace, overrides }: InitialOptions) => {
    ...
    this.cache = createStorage<PreferencesStorage>('local', this.storagePrefix);
  ...
  private loadFromCache(): null | Preferences {
    return this.cache.get(STORAGE_KEYS.MAIN);
  }
  ...
  private saveToCache(preference: Preferences) {
    this.cache.set(STORAGE_KEYS.MAIN, preference);
    this.cache.set(STORAGE_KEYS.LOCALE, preference.app.locale);
    this.cache.set(STORAGE_KEYS.THEME, preference.theme.mode);
  }
